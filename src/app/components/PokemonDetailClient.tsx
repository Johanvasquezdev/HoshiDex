  ...
  return (
    <div className="flex flex-col gap-3">
      <Link
        ...
        className="rounded-xl border border-slate-200 bg-slate-50 px-4 py-3 text-sm font-black text-slate-800 transition hover:border-red-200 hover:bg-red-50 dark:border-white/10 dark:bg-white/5 dark:text-slate-100 dark:hover:border-red-400/40 dark:hover:bg-red-500/10"
      >
        {node.displayName}
      </Link>
    ...
    <div className="min-h-screen overflow-x-hidden bg-slate-50 pb-20 dark:bg-[#07090f]">
            ...
            <div>
              ...
              <div className="mt-4 flex flex-wrap gap-3">
                {pokemon.types.map((type) => (
                  <span key={type} className="rounded-full border border-white/20 bg-white/20 px-5 py-2 text-sm font-black capitalize shadow-sm backdrop-blur-md">
                    {type}
                  </span>
                ))}
              </div>
      ...
      <div className="relative z-20 mx-auto -mt-40 max-w-5xl px-4 sm:px-6">
                ...
                {pokemon.abilities.map((ability) => (
                  <div key={ability.name} className="rounded-2xl border border-slate-100 bg-slate-50 p-4 shadow-sm dark:border-white/10 dark:bg-white/5 dark:shadow-black/20">
                    <div className="flex items-center justify-between gap-3">
                      <h3 className="text-lg font-black text-slate-800 dark:text-white">{ability.displayName}</h3>
                      {ability.isHidden && (
        ...
        <div className="mb-10 grid gap-10 lg:grid-cols-2">
          {pokemon.evolution && (
            <section className="rounded-[2rem] border border-slate-100 bg-white p-6 shadow-xl sm:p-8 dark:border-white/10 dark:bg-[#0d1324] dark:shadow-black/30">
              <h2 className="mb-5 text-2xl font-black text-slate-900 dark:text-white">Evolution Chain</h2>
              <EvolutionTree node={pokemon.evolution} />
            </section>
          )}
          <section className="rounded-[2rem] border border-slate-100 bg-white p-6 shadow-xl sm:p-8 dark:border-white/10 dark:bg-[#0d1324] dark:shadow-black/30">
            <h2 className="mb-5 text-2xl font-black text-slate-900 dark:text-white">Forms & Variations</h2>
            <div className="grid grid-cols-2 gap-4 sm:grid-cols-3">
              {pokemon.varieties.map((variety) => (
                <Link
                  key={variety.name}
                  href={`/pokemon/${variety.idOrName}`}
                  className="group rounded-2xl border border-transparent p-3 text-center transition hover:border-slate-100 hover:bg-slate-50 dark:hover:border-white/10 dark:hover:bg-white/5"
                >
                  <span className="mx-auto mb-2 grid h-24 w-24 place-items-center rounded-full bg-slate-100 shadow-inner dark:bg-white/10 dark:shadow-black/20">
                    <img
                      src={variety.sprite}
                  ...
                  <span className="text-sm font-black text-slate-700 dark:text-slate-200">{variety.displayName}</span>
                </Link>
              ))}
            </div>
          </section>
